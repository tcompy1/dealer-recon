...
| Configuration-model conclusion | The existing Hurst/Acura model covers FW's store label, account label, output naming, BOA cleaning, total labels, and DT-only placement in principle, but a single `dealertrack_account_column` is insufficient. FW requires `dealertrack_amount_columns`, `dealertrack_excluded_account_columns`, and likely `merged_sheet_label_aliases` for accepted artifact validation. |
...
1. Store workflow matrix first, now including FW as the first multi-account Dealertrack variant.
2. Merged spreadsheet fidelity remains first implementation priority. Hurst and Acura stay the baseline; FW should be added next only after the store config model supports multi-column DT aggregation.
3. Store configuration model third, expanded for `dealertrack_amount_columns`, `dealertrack_excluded_account_columns`, and accepted merged-label aliases.
4. FP REC generation from merged artifact and store config fourth, including FW's aggregated `2100` GL total semantics.
...
| Add store config for Hurst, Acura, and FW | Introduces explicit config for `store_key`, `display_name`, `merged_sheet_label`, `dealertrack_amount_columns`, `dealertrack_excluded_account_columns`, `dealertrack_account_label`, `output_filename_prefix`, BOA description behavior, totals row labels, and DT-only placement rule. No `HURST`, `ACURA`, `FW`, `FORT WORTH`, `2100`, `2101`, `2101S`, or `324` output behavior is hardcoded in shared presenter logic. |
...
| Add FW golden tests | Adds golden fixtures from FW February, March, and April raw/merged evidence. Tests assert raw DT columns `Control`, `Description`, `2100`, `2101`, `2101S`, `2110`; merged columns with accepted `FW`/`FORT WORTH` labels and output `2100`; BOA totals excluding `$2,000,000.00` straightline; DT totals equal `2100 + 2101 + 2101S` and exclude `2110`. |
| Add FW raw preprocessing-to-merged tests | Given FW raw BOA and raw Dealertrack files, preprocessing produces the same matched, BOA-only, DT-only, BOA total, and aggregated DT total counts derived from Tara's accepted FW merged CSVs. |
